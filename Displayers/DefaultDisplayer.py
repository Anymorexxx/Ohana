...
class DefaultDisplayer(IDisplayer):
    ...
    async def ChanelCreated(self, channel : TextChannel):
        ...
        description=f'Канал создан: {channel.created_at.strftime("%d.%m.%Y %H:%M:%S")}', color=self.color)
    ...
    async def RoleCreate(self, role):
    ...
    async def RoleDelete(self, role):
    ...
    async def RoleEdit(self, before, after):
    ...
    async def MemberAddedRole(self, added_roles, member, befor):
        ...
        await self.ShowModerUpdatedRole(added_role, embed, befor)
        embed.add_field(name = 'Дата:ᅠ ᅠ ᅠ Время:', value = f'```fix\n{datetime.now().strftime("%d.%m.%Y %H:%M:%S")}\n```', inline=False)
        await channel.send(embed=embed)

    async def MemberRemovedRoles(self, removed_roles, member, befor):
        ...
        await self.ShowModerUpdatedRole(removed_role, embed, befor)
        embed.add_field(name = 'Дата:ᅠ ᅠ ᅠ Время:', value = f'```fix\n{datetime.now().strftime("%d.%m.%Y %H:%M:%S")}\n```', inline=False)
        await channel.send(embed=embed)

    async def ShowModerUpdatedRole(self, changed_role : discord.Role, embed, before):
    ...
    async def MemberNicknameUpdated(self, befor, after, entry):
        channel = self.bot.get_channel(self.log)
        embed = discord.Embed(description = f'**Изменён ник.**', color = self.color)
        embed.add_field(name = 'ᅠ ᅠ ᅠ', value = f'`{befor.nick}` `-->` `{after.nick}`', inline=False)
